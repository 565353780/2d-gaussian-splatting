...
def compute_gaussian_score_fastgs(camlist, gaussians, pipe, bg, args, DENSIFY = False):
    ...
    for view in range(len(camlist)):
        ...
        # 使用 metric_map 调用 render，获取 accum_metric_counts
        get_flag = True
        metric_map_flat = metric_map.reshape(-1)  # 展平为 (H*W,)
        render_pkg = render(my_viewpoint_cam, gaussians, pipe, bg, get_flag=get_flag, metric_map=metric_map_flat)

        # 从 render_pkg 获取 accum_metric_counts
